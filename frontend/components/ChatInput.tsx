...
export function ChatInput({ onSubmit, isSending }: ChatInputProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 border-t border-default-200 p-4">
      <TextField value={subject} onChange={setSubject} isDisabled={isSending} className="flex flex-col gap-1">
      ...
      <div className="flex items-end gap-2">
        ...
        >
          ...
          <TextArea fullWidth placeholder="Type the customer's message here…" maxLength={MAX_BODY_LENGTH} rows={2} />
          <Description className="text-xs text-muted">
            {body.length} / {MAX_BODY_LENGTH}
          </Description>
        </TextField>
        <Button type="submit" variant="primary" isDisabled={isSending || !body.trim()}>
